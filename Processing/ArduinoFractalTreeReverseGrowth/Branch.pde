...
class Branch {
  ...
  public void setGrowthPositive(boolean setGrowthPos){
    setGrowthPositive(this, setGrowthPos);
  }
  
  private void setGrowthPositive(Branch branch, boolean setGrowthPos){
    
    for (Branch childBranch : branch.childrenBranches) {
      setGrowthPositive(childBranch, setGrowthPos);
    }
    
    if(setGrowthPos){
      branch.growthStep = abs(branch.growthStep);
    }else{
      branch.growthStep = -1 * abs(branch.growthStep);
      branch.growthAmt += branch.growthStep;
      println("branch.growthStep=" + branch.growthStep + " branch.growthAmt=" + branch.growthAmt);
    }
  }
  ...
  private void draw(Branch branch, int maxDepth) {
    ...
    if(branch.growthAmt >= 1f && branch.growthStep > 0){
      ...
      println("animating smaller: " + " branch.growthAmt=" + branch.growthAmt + " branch.growthStep=" + branch.growthStep);
      //branch.growthPt = PVector.lerp(branch.end, branch.begin, branch.growthAmt);
      branch.growthPt = PVector.lerp(branch.begin, branch.end, branch.growthAmt);
      branch.growthAmt += branch.growthStep;
