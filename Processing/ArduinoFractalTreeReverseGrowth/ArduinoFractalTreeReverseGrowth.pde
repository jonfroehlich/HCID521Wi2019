...
void serialEvent (Serial myPort) {
  try {
    ...
    if (inString != null) {
      ...
      if(newDesiredTreeDepth >= _desiredTreeDepth){
        if(setGrowthNegative == false){
          _tree.setGrowthPositive(true); 
        }
      }else{
        _tree.setGrowthPositive(false); 
        setGrowthNegative = true;
        println("Set growth negative");
      }
